...
export function DeploymentWizard() {
  ...
  // Auto-advance steps based on progress and deployment results
  useEffect(() => {
    // Move to deploy step when contracts are deployed
    if (deployedContracts && step !== 'deploy' && step !== 'registry' && step !== 'complete') {
      setStep('deploy');
    }
    
    // Move to registry step after deployment completes
    if (deployedContracts && !deploying && step === 'deploy') {
      setStep('registry');
    }
    
    // Move to complete step when registration is done
    if (deploymentResult && step === 'registry') {
      setStep('complete');
    }
    
    // Also check progress.stage for completion
    if (progress?.stage === 'complete' && step !== 'complete') {
      setStep('complete');
    }
  }, [deployedContracts, deploying, deploymentResult, progress, step]);
  ...
  const handleRegisterInRegistry = async () => {
    ...
    // Note: registerInRegistry will handle chain switching automatically
    // Don't block here - let it switch to Sepolia internally
    ...
    // Step will be advanced automatically by useEffect when deploymentResult is set or progress.stage === 'complete'
  };
              ...
              <AlertDescription>
                <div className="space-y-2 text-sm">
                  ...
                  <ul className="list-disc ml-4 space-y-1">
                    ...
                    <li><strong className="text-destructive">MUST manually increase gas limit</strong> in MetaMask to <strong>30,000,000</strong> (30M) for each contract</li>
                  ...
                  <p className="mt-2 font-semibold text-primary">
                    💡 Tip: In MetaMask, click "Edit" → "Advanced gas fees" → Set "Gas limit" to 30000000
                  </p>
                </div>
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-3 bg-primary/5 border border-primary/20 rounded-lg p-4">
              ...
              {progress.txHash && (
                <div className="text-sm space-y-1">
                  ...
                  <a
                    href={`https://${network === 'Sepolia' ? 'sepolia.' : network === 'Arbitrum Sepolia' ? 'sepolia.' : ''}${network === 'Story Testnet' ? 'aeneid.storyscan.io' : 'etherscan.io'}/tx/${progress.txHash}`}
                    target="_blank"
              ...
              <>
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 mb-4">
                  <p className="text-sm text-foreground">
                    ...
                    to register in the global registry. Click the button below to switch automatically.
                  </p>
                </div>
                ...
                <Button
                  ...
                  className="w-full gradient-primary"
                  disabled={registering || switching}
                >
                  {registering || switching ? (
                    <>
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                      {registering ? 'Switching network and registering...' : 'Waiting for network switch...'}
                    </>
                  ) : (
                    'Switch to Ethereum Sepolia and Register'
                  )}
                </Button>
                ...
                <Button
                  onClick={() => {
                    setStep('deploy');
                    // Clear the deployed contracts to force a fresh deployment
              ...
              <div className="space-y-3 bg-primary/5 border border-primary/20 rounded-lg p-4 mb-4">
                ...
                {progress.txHash && (
                  <div className="text-sm space-y-1">
                    <div className="text-muted-foreground">Transaction Hash:</div>
                    <a
                      href={`https://${progress.stage === 'configuring-evvm' ? (network === 'Story Testnet' ? 'aeneid.storyscan.io' : 'sepolia.etherscan.io') : 'sepolia.etherscan.io'}/tx/${progress.txHash}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="font-mono text-primary hover:underline break-all text-xs"
                    >
